...
export function MobileTabBar({ user }: { user: SessionUser }) {
  ...
  return (
    <nav
      className="pb-safe-tabbar fixed inset-x-0 bottom-0 z-30 flex border-t border-neutral-200 bg-white sm:hidden"
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className={`flex flex-1 flex-col items-center gap-0.5 pt-2 text-[11px] font-medium ${
